...
import { PanelDeviceType } from '../vivint_dictionary.json'
...
class Lock extends Device {
  ...
  async setTargetState(targetState, callback) {
    ...
    try {
      ...
      callback(
        new Error(`An error occurred while setting the door lock state: ${err}`)
      )
    }
  }
  ...
  notify() {
    ...
    if (this.service) {
      ...
      this.service.updateCharacteristic(
        ...
        state == this.Characteristic.LockTargetState.SECURED
      )
    }
  }

  static appliesTo(data) {
    return data.Type == PanelDeviceType.DoorLock
  }
