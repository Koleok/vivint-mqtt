...
class MotionSensor extends Device {
  constructor(accessory, data, config, log, homebridge, vivintApi) {
    ...
    this.config_motionDetectedOccupancySensorMs =
      (config.motionDetectedOccupancySensorMins || 0) * 60 * 1000
  ...
  getOccupancyState() {
    ...
    else return this.Characteristic.OccupancyDetected.OCCUPANCY_NOT_DETECTED
  }
  ...
  static appliesTo(data) {
    return (
      ...
      (data.EquipmentCode == EquipmentCode.PIR1_MOTION ||
        data.EquipmentCode == EquipmentCode.PIR2_MOTION ||
        data.EquipmentCode == EquipmentCode.EXISTING_MOTION_DETECTOR)
    )
  }
  ...
  static addServices(accessory, Service, config) {
    ...
    if (config.motionDetectedOccupancySensorMins > 0) {
      accessory.addService(
        new Service.OccupancySensor(accessory.context.name + ' Occupancy')
      )
    }
  }
}
