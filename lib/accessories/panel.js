...
import { SecurityState, PanelDeviceType } from '../vivint_dictionary.json'
...
class Panel extends Device {
  constructor(accessory, data, config, log, homebridge, vivintApi) {
    ...
    this.VIVINT_TO_HOMEKIT = {
      ...
      [SecurityState.ARMING_AWAY_IN_EXIT_DELAY]:
        this.Characteristic.SecuritySystemCurrentState.AWAY_ARM,
      [SecurityState.ARMING_STAY_IN_EXIT_DELAY]:
        this.Characteristic.SecuritySystemCurrentState.STAY_ARM,
      [SecurityState.ARMED_STAY]:
        this.Characteristic.SecuritySystemCurrentState.STAY_ARM,
      [SecurityState.ARMED_AWAY]:
        this.Characteristic.SecuritySystemCurrentState.AWAY_ARM,
      [SecurityState.ARMED_STAY_IN_ENTRY_DELAY]:
        this.Characteristic.SecuritySystemCurrentState.STAY_ARM,
      [SecurityState.ARMED_AWAY_IN_ENTRY_DELAY]:
        this.Characteristic.SecuritySystemCurrentState.AWAY_ARM,
      [SecurityState.ALARM]:
        this.Characteristic.SecuritySystemCurrentState.ALARM_TRIGGERED,
      [SecurityState.ALARM_FIRE]:
        this.Characteristic.SecuritySystemCurrentState.ALARM_TRIGGERED,
      [SecurityState.DISABLED]:
        this.Characteristic.SecuritySystemCurrentState.DISARMED,
      [SecurityState.WALK_TEST]:
        this.Characteristic.SecuritySystemCurrentState.DISARMED,
    }
    ...
    this.service
      ...
      .on('set', async (state, callback) =>
        this.setTargetState(state, callback)
      )
  ...
  async setTargetState(targetState, callback) {
    ...
    try {
      ...
      if (
        ...
        await this.vivintApi.setPanelState(
          this.HOMEKIT_TO_VIVINT[
            this.Characteristic.SecuritySystemTargetState.DISARM
          ]
        )
      ...
      callback(
        new Error(`An error occurred while setting the panel state: ${err}`)
      )
    }
  }
  ...
  static appliesTo(data) {
    return data.Type == PanelDeviceType.Panel
  }
