...
class SmokeSensor extends Device {
  ...
  getSensorState() {
    ...
    else return this.Characteristic.SmokeDetected.SMOKE_NOT_DETECTED
  }
  ...
  static appliesTo(data) {
    return (
      ...
      (data.EquipmentCode == EquipmentCode.FIREFIGHTER_AUDIO_DETECTOR ||
        data.EquipmentCode == EquipmentCode.HW_SMOKE_5808W3 ||
        data.EquipmentCode == EquipmentCode.EXISTING_SMOKE ||
        data.EquipmentCode == EquipmentCode.SMKE1_SMOKE_CANADA ||
        data.EquipmentCode == EquipmentCode.SMKE1_SMOKE ||
        data.EquipmentCode == EquipmentCode.VS_SMKT_SMOKE_DETECTOR ||
        data.EquipmentCode == EquipmentCode.SMKT2_GE_SMOKE_HEAT ||
        data.EquipmentCode == EquipmentCode.SMKT3_2GIG ||
        data.EquipmentCode == EquipmentCode.SMKT6_2GIG)
    )
  }
