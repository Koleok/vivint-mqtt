...
class Thermostat extends Device {
  constructor(accessory, data, config, log, homebridge, vivintApi) {
    ...
    this.service
      ...
      .on('set', async (state, callback) =>
        this.setTargetHeatingCoolingState(state, callback)
      )
    ...
    this.service
      ...
      .on('set', async (target, callback) =>
        this.setTargetTemperature(target, callback)
      )
    ...
    this.service
      ...
      .on('get', async (callback) =>
        this.getCoolingThresholdTemperature(callback)
      )
      .on('set', async (target, callback) =>
        this.setCoolingThresholdTemperature(target, callback)
      )
    ...
    this.service
      ...
      .on('get', async (callback) =>
        this.getHeatingThresholdTemperature(callback)
      )
      .on('set', async (target, callback) =>
        this.setHeatingThresholdTemperature(target, callback)
      )
  ...
  getOperatingState() {
    switch (this.data.OperatingState) {
      case OperatingStates.IDLE:
        return this.Characteristic.CurrentHeatingCoolingState.OFF
      case OperatingStates.HEATING:
        return this.Characteristic.CurrentHeatingCoolingState.HEAT
      case OperatingStates.COOLING:
  ...
  getOperatingMode() {
    switch (this.data.OperatingMode) {
      case OperatingModes.OFF:
        return this.Characteristic.TargetHeatingCoolingState.OFF
      case OperatingModes.HEAT:
        return this.Characteristic.TargetHeatingCoolingState.HEAT
      case OperatingModes.COOL:
        return this.Characteristic.TargetHeatingCoolingState.COOL
      case OperatingModes.AUTO:
        return this.Characteristic.TargetHeatingCoolingState.AUTO
      case OperatingModes.ECO:
  ...
  async setTargetHeatingCoolingState(state, callback) {
    ...
    switch (state) {
      case this.Characteristic.TargetHeatingCoolingState.OFF:
        setValue = OperatingModes.OFF
        break
      case this.Characteristic.TargetHeatingCoolingState.HEAT:
        setValue = OperatingModes.HEAT
        break
      case this.Characteristic.TargetHeatingCoolingState.COOL:
        setValue = OperatingModes.COOL
        break
      case this.Characteristic.TargetHeatingCoolingState.AUTO:
        setValue = OperatingModes.AUTO
        break
    }
    ...
    try {
      ...
      this.log.error(
        'Failure setting thermostat Target Heating/Cooling state:',
        setValue,
        err
      )
      callback(
        new Error(
          `Failure setting thermostat Target Heating/Cooling state: ${setValue} ${err}`
        )
      )
    }
  }
    ...
    switch (this.getOperatingMode()) {
      ...
      case this.Characteristic.TargetHeatingCoolingState.HEAT:
        try {
          callback()
          await debounce(
            this.vivintApi.setThermostatHeatSetPoint(this.id, temperature),
            DEBOUNCE_SECONDS * 1000,
            { leading: true }
          )
        } catch (err) {
          this.log.error(
            'Failure setting target temperature:',
            temperature,
            err
          )
          callback(
            new Error(
              `An error occurred while setting target temperature: ${temperature} ${err}`
            )
          )
        }
        break
      case this.Characteristic.CurrentHeatingCoolingState.COOL:
        try {
          callback()
          await debounce(
            this.vivintApi.setThermostatCoolSetPoint(this.id, temperature),
            DEBOUNCE_SECONDS * 1000,
            { leading: true }
          )
        } catch (err) {
  ...
  async setCoolingThresholdTemperature(value, callback) {
    try {
      callback()
      await debounce(
        this.vivintApi.setThermostatCoolSetPoint(this.id, value),
        DEBOUNCE_SECONDS * 1000,
        { leading: true }
      )
    } catch (err) {
      this.log.error(
        'Failure setting Cooling Threshold temperature:',
        value,
        err
      )
      callback(
        new Error(
          `An error occurred while setting Cooling Threshold temperature: ${value} ${err}`
        )
      )
    }
  }
  ...
  async setHeatingThresholdTemperature(value, callback) {
    try {
      callback()
      await debounce(
        this.vivintApi.setThermostatHeatSetPoint(this.id, value),
        DEBOUNCE_SECONDS * 1000,
        { leading: true }
      )
    } catch (err) {
      this.log.error(
        'Failure setting Heating Threshold temperature:',
        value,
        err
      )
      callback(
        new Error(
          `An error occurred while setting Heating Threshold temperature: ${value} ${err}`
        )
      )
    }
  }
  ...
  async setFanOn(fanOn, callback) {
    ...
    try {
      ...
      callback(
        new Error(
          `An error occurred while setting Fan state: ${fanState} ${err}`
        )
      )
    }
  }
  ...
  async getTemperatureDisplayUnits(callback) {
    return callback(
      null,
      this.isCelsius()
        ? this.Characteristic.TemperatureDisplayUnits.CELCIUS
  ...
  notify() {
    if (this.service) {
      ...
      this.service.updateCharacteristic(
        ...
        this.getTargetTemperatureValue()
      )
      this.service.updateCharacteristic(
        this.Characteristic.CurrentRelativeHumidity,
        this.getHumidityValue()
      )
      this.service.updateCharacteristic(
        this.Characteristic.CoolingThresholdTemperature,
        this.getCoolingThresholdTemperatureValue()
      )
      this.service.updateCharacteristic(
        this.Characteristic.HeatingThresholdTemperature,
        this.getHeatingThresholdTemperatureValue()
      )
      this.service.updateCharacteristic(
  ...
  static appliesTo(data) {
    return data.Type == PanelDeviceType.Thermostat
  }
