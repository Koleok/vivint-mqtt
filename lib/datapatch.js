...
function dataPatchArr(data, patch) {
  for (let idx = 0; idx <= patch.length - 1; idx++) {
    if (!idx in data) {
      ...
      if (!dataPatchObj(data[idx], patch[idx])) return false
    } else {
      data[idx] = patch[idx]
    }
  }
...
function dataPatchObj(data, patch) {
  ...
  for (key in patch) {
    ...
    if (selector.length > 1) {
      ...
      if (!dataPatchObj(data[head], restPatch)) return false
    } else {
      if (patch[key] instanceof Array) {
        if (!dataPatchArr(data[key], patch[key])) return false
      } else {
        data[key] = patch[key]
      }
    }
