...
class Device {
  ...
  getBatteryLevelValue() {
    return Object.is(this.data.BatteryLevel, undefined)
      ? 100
  ...
  handleSnapshot(data) {
    if (data.Id != this.id) throw 'This snapshot does not belong to this device'
    this.data = data
    this.notify()
  }
   ...
   */
  handlePatch(patch) {
    if (patch.Id != this.id) throw 'This patch does not belong to this device'
  ...
  notify() {
    if (this.batteryService) {
      this.batteryService.updateCharacteristic(
        ...
        this.getLowBatteryState()
      )
      this.batteryService.updateCharacteristic(
        this.Characteristic.BatteryLevel,
        this.getBatteryLevelValue()
      )
    }
  }
