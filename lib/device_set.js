...
import ContactSensor from './accessories/contact_sensor.js'
import SmokeSensor from './accessories/smoke_sensor.js'
import CarbonMonoxideSensor from './accessories/carbon_monoxide_sensor.js'
import MotionSensor from './accessories/motion_sensor.js'
import Lock from './accessories/lock.js'
import Thermostat from './accessories/thermostat.js'
import GarageDoor from './accessories/garage_door.js'
import Panel from './accessories/panel.js'
import Camera from './accessories/camera.js'
import LightSwitch from './accessories/light_switch.js'
import DimmerSwitch from './accessories/dimmer_switch.js'
...
function DeviceSetModule(config, log, homebridge, vivintApi) {
  ...
  class DeviceSet {
    ...
    handleSnapshot(deviceData, timestamp) {
      ...
      let panelData = deviceData.Devices.find(
        (dvc) => dvc.Id == this.panel_DeviceId
      )
      if (panelData !== null) {
      ...
      if (message.Data) {
        if (message.Data.PlatformContext.Timestamp < this.lastSnapshotTime) {
          log.warn(
            ...
            message.Data.PlatformContext.Timestamp,
            '<',
        ...
        //Jammed lock notification
          ...
          message.Type === ObjectType.InboxMessage &&
          message.Data != null &&
          message.Data.Subject != null &&
          message.Data.Subject.indexOf('failed to lock') !== -1
        ) {
          const lockName =
            message.Data.Subject.split('Alert: ')[1].split(' failed to lock')[0]
          var lockDevice = this.devices.find((device) => {
            return (
              device.data.Type === PanelDeviceType.DoorLock &&
              device.name === lockName
            )
          })
          if (lockDevice) {
            message.Data.Devices = [
    ...
    static createDeviceAccessory(data) {
      ...
      if (irrelevantDeviceTypes.indexOf(data.Type) != -1) {
        log.debug(
          `Ignored unuseable device [Type]:${data.Type} [Data]:`,
      ...
      if (!deviceClass) {
        log.info(
          `Device not (yet) supported [ID]:${data.Id} [Type]:${data.Type} [EquipmentCode]:${data.EquipmentCode} [Name]:${data.Name}`
        )
        log.debug(
          'Unsupported device found! [Data]:',
          JSON.stringify(data, undefined, 4)
        )
        return null
      }
      ...
      if (config_IgnoredDeviceTypes.indexOf(data.Type) != -1) {
        log.info(
          `Ignored device [ID]:${data.Id} [Type]:${data.Type} [EquipmentCode]:${data.EquipmentCode} [Name]:${data.Name}`
        )
        return null
      }

      let serial =
        (data.SerialNumber32Bit || 0).toString(16).padStart(8, '0') +
        ':' +
        (data.SerialNumber || 0).toString(16).padStart(8, '0') +
        ':' +
        data.Id

      let category =
        (deviceClass.inferCategory &&
          deviceClass.inferCategory(data, Accessory)) ||
        Accessory.Categories.OTHER

      var manufacturer = 'Vivint'
      var model =
        data.EquipmentCode !== undefined
          ? vivintApi.getDictionaryKeyByValue(EquipmentCode, data.EquipmentCode)
          : deviceClass.name
      ...
      let informationService = accessory.getService(
      ...
      if (data.CurrentSoftwareVersion || data.SoftwareVersion) {
        informationService.setCharacteristic(
          ...
          data.CurrentSoftwareVersion || data.SoftwareVersion
        )
      }
...
export default DeviceSetModule
