...
import DeviceSetModule from '../lib/device_set.js'
...
class MockService {
  ...
  getCharacteristic(characteristic) {
    if (!(characteristic.name in this.characteristics))
      this.characteristics[characteristic.name] = new Characteristic(
        characteristic
      )
  ...
  hap: {
    ...
    Characteristic: {
      ...
      LockCurrentState: {
        ...
        SECURED: 'LCS_SECURED',
        UNSECURED: 'LCS_UNSECURED',
      },
      LockTargetState: {
        name: 'LockTargetState',
        SECURED: 'LTS_SECURED',
        UNSECURED: 'LTS_UNSECURED',
      },
      MotionDetected: {
        name: 'MotionDetected',
        SECURED: 'SECURED',
      ...
      OccupancyDetected: {
        ...
        OCCUPANCY_NOT_DETECTED: 'OCCUPANCY_NOT_DETECTED',
      },
    },
  },
}
...
describe('DeviceSet', function () {
  ...
  beforeEach(() => {
    ...
    DeviceSet = DeviceSetModule(
      { motionDetectedOccupancySensorMins },
      console.log,
      MockHomebridge,
      {},
      {},
  ...
  describe('#constructor', function () {
    it('populates the devices with the given delegators', function () {
      var deviceSet = deviceSetFromData(
        [mockWindowSensor(), mockDoorSensor(), mockDoorLock()],
        snapshotTs
      )
      equal(3, deviceSet.devices.length)
      console.log(deviceSet)
      equal('ContactSensor', deviceSet.devicesById[28].constructor.name)
      equal('ContactSensor', deviceSet.devicesById[42].constructor.name)
      equal('Lock', deviceSet.devicesById[24].constructor.name)
    })
  })
  ...
  describe('#motionSensorOccupancy', function () {
    it('turns on occupancy sensor for the configured duration when the motion sensor actives', () => {
      ...
      let occupancyDetected = occupancy.getCharacteristic(
        Characteristic.OccupancyDetected
      )
      equal(1, MockSetInterval.getTimers().length)
      let occupancyInterval = MockSetInterval.getTimers()[0]
      equal(60000, occupancyInterval.frequency)

      // defaults to off

      equal(
        Characteristic.OccupancyDetected.OCCUPANCY_NOT_DETECTED,
        occupancyDetected.triggerGetSync()
      )
      ...
      deviceSet.handleMessage({
        message: {
          da: {
            d: [{ _id: 45, s: true }],
            plctx: { ts: '2018-06-16T06:59:09.198000' },
          },
        },
      })
  ...
  describe('#handleSnapshot', function () {
    it('receives a new snapshot of data and publishes to homebridge', function () {
      ...
      equal(
        basementShopWindow.contactSensorValue(),
  ...
  describe('#handleMessage', function () {
    it('updates the data and publishes an update to homebridge', function () {
      var deviceSet = deviceSetFromData(
        [mockWindowSensor(), mockDoorSensor()],
        snapshotTs
      )
      var basementShopWindow = deviceSet.devicesById[42]
      let Characteristic = MockHomebridge.hap.Characteristic
